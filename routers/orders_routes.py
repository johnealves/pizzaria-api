...
from schemas.schemas import ItemOrderSchema, OrderSchema, ResponseOrdersSchema
...
async def list_orders(session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    if not user.admin:
        raise HTTPException(status_code=401, detail="User don't have authorization to view orders")
    else:
        orders = session.query(Order).all()
        return {"orders": orders}

@orders_router.get('/my', response_model=list[ResponseOrdersSchema])
async def get_order_by_user(session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    orders = session.query(Order).filter(Order.user_id == user.id).all()
    
    return orders

@orders_router.get('/{order_id}')
async def get_order_by_id(order_id: str, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
...
async def add_item_order(order_id: str,
    ...
    item_order =  OrderItem(item_order_schema.quantity,
...
async def remove_item_order(item_order_id: str,
                           user: User = Depends(get_current_user),
                           session: Session = Depends(get_session)):
...
async def update_order(order_id: int,
    ...
    if not user.admin and user.id != selected_order.user_id:
        raise HTTPException(status_code=401, detail="User don't have authorization edit this order")
